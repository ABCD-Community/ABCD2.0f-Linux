...
//#endif
    ...
    for (nwo=nwc=0, occ=cnt=1, p=(UCHR *)(keyp=fmtap); *keyp; ) {
     ...
     fst_wluptrmp=vtrmp[fst_wlupitrm];
        ...
        pfxp=q; pfxl=0;
#if UTF8
        for (upcase_length = 0; *q && *q != '/'; q++)
        {
        	upcase_length++;
			if (pfxl < LE2) pfxl++;
        }
		uppercase_utf8_length(isisuctab_utf8,keyp,upcase_length);
#else
        for (; *q && *q != '/'; q++) {
...
#endif
    int len,n,bplen,bklen,left,plen0,klen0,nkeyp;
    int xrc;
...
/* NEW by EdS to avoid truncating in middle of multi-byte char */
#if UTF8
// printf("truncate before - %s %d\n",keyp,klen);
///klen=strlen(keyp);
if (strlen(keyp) > LE2X) {
   nkeyp = (char *) malloc(LE2X);
//      newkeyp=malloc(LE2X);
   keyp=utf8cpy(nkeyp, keyp, LE2X-plen );
   klen=strlen(keyp);
// printf("truncate  after - %s %d\n",keyp,klen);
}
#endif // UTF8
/* end of NEW EdS */
...
#if UTF8
	len = bplen;
    memcpy(line,bpfx,bplen);
	uppercase_utf8_length(isisuctab_utf8,line,bplen);
	p=line+bplen;
	int max;
int i;
int point;
...
#if UTF8
	{
		// If uppercase wanted, use mpu,mhu,mdu in the fst
	    for (n=0; n < bklen; n++, len++)
	        if (len < LE2X) *p++ = bkey[n]; else break;
     	/*
     	// Forced uppercase, if it would work like non utf-8 style
		memcpy(p,bkey,bklen);
		len = bklen < LE2X ? bklen : LE2X;
		uppercase_utf8_length(isisuctab_utf8,p,len);
		*/
	}
#else
    for (n=0; n < bklen; n++, len++)
