...
#if UTF8
			q = word_utf8(q,word);
			wlen += strlen_utf8(word);
			hlen += strlen_utf8(word);
...
#define LE2X    LE2
#endif
#if UTF8
	//printf("pfxp=[%.140s], plen=%d\n", pfxp, plen);
	//printf("         keyp=[%.140s], klen=%d\n", keyp, klen);
	truncate_utf8(keyp, LE2X - plen);
	klen = strlen(keyp);
	//printf("truncate keyp=[%.140s], klen=%d\n", keyp, klen);
#endif // UTF8
    ...
    *p++='\n'; *p='\0';
...
#if UTF8
	{
		memcpy(p,bkey,bklen);
		len += bklen;
	     	// Forced uppercase, uncomment next line if it would work like non utf-8 style
