...
#
echo $THIS_PATH/lin/hldg_db.sh $THIS_CISIS $TITLE $KEY $TAG_RESUMIDO $HLDG $LOG_FILE $THIS_PATH $THIS_PROC_TMP_PATH
...
$THIS_CISIS/mx $ALLFACIC btell=0 \x22$KEY=$" lw=99999 "proc='d920a920~',s('000000000000000000000000000000',v920),'~'"  "proc='d920a920~',mid(v920,size(v920)-30+1,30),'~'" "pft=replace(s(v920,'|',v911,'|',v912,'|',v913,'|',v914,'|',if instr(':PT:S:NE:BI:IN:AN:',s(':',v916,':'))>0 then v916 else '' fi,'|',v910,'|',if instr(':PT:S:NE:BI:IN:AN:',s(':',v916,':'))>0 then 'S' else 'R' fi),'| |','||')/" now | sort > $THIS_PROC_TMP_PATH/ORD.txt
fi
...
echo 	$THIS_CISIS/mx $HLDG btell=0 "tit=$KEY" "proc='d*','a10~',v10,'~',"  "proc='a$TAG_ID~$KEY~a$TAG_TIME~^s$TIME_START^e$TIME_END~'" "proc='a$TAG_TIME~^d',f(val(s(v$TAG_TIME^e*11))-val(s(v$TAG_TIME^s*11)),1,0),'~'" "proc=@$PROC_FILE" "proc=@$PROC_FILE_CHECK" copy=$HLDG now -all >$LOG_FILE
